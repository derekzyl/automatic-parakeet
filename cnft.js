...
async function nftLoader(page, url) {
  await page.evaluate(() => {
    ...
    if (document.querySelector(".item--description-text")) {
      nft.creator =
        ...
        document
          .querySelector(".item--description-text")
          .children[0].children[0].children[0].innerText.split("Created by")[0]
          .trim();
    }
  ...
  //     .then((data) =>{
  //   window.localStorage.setItem("o", JSON.stringify(data.message));
  //        log(JSON.stringify(data.message))})
  //        log(data.message)
  //     .catch(function(res){ 
  //       log(JSON.stringify(res))
  //       log(res.error)
  //   window.localStorage.setItem("e", JSON.stringify(res));
  //     });
